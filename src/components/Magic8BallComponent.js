...
import { Question } from './ApiCalls.js';
import { Dropdown } from 'flowbite-react';

export default function Magic8BallComponent() {
    const [questionInput, QuestionInput] = useState('');
    const [result, setResult] = useState('');

    const M8B = async () => {
        const response = await Question(questionInput);
        setResult(response);
    }
        ...
        <div>
                ...
                <Navbar fluid rounded className='customBg w-4/5 navbarHeight'>
                    <Navbar.Brand >
                        <a className="flex items-center space-x-3 rtl:space-x-reverse">
                            <span className="self-center font50 text-wrap font-semibold whitespace-nowrap dark:text-white">Magic 8 Ball</span>
                        </a>
                    </Navbar.Brand>
                    <Navbar.Toggle />
                    <Dropdown label="Other Pages" className='w-11/12 font-bold' dismissOnClick={false}>
                        ...
                        <Dropdown.Item as={Link} to='/Magic8Ball' className='block py-5 px-3 text-white hover:text-gray-900 bg-blue-700 rounded dark:bg-blue-600 font30 text-wrap'>Magic 8 Ball</Dropdown.Item>
            ...
            <div class="flex justify-center mt-12">
                <div class="grid grid-cols-1 gap-8 w-4/5 font30">
                    <div class="flex justify-center px-5 py-2.5 m-2">Give the computer a question and it will respond back with an answer that will probably best suit your question.</div>
                    <div class="flex justify-center px-5 py-2.5 m-2"><input type="text" placeholder="Enter Question Here" class="rounded-md w-96" onChange={e => QuestionInput(e.target.value)} /></div>
                    <div class="flex justify-center px-5 py-2.5 m-2"><button type="button" class="customBg hover:bg-gray-400 font-medium rounded-lg px-5 m-2" onClick={() => M8B()}>Submit</button></div>
