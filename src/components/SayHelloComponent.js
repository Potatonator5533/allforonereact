...
import { SayHello } from './ApiCalls.js';
import { Dropdown } from 'flowbite-react';

export default function SayHelloComponent() {
    const [sayHelloName, SayHelloName] = useState('');
    const [result, setResult] = useState('');

    const SH = async () => {
        const response = await SayHello(sayHelloName);
        setResult(response);
    }
    ...
    return (
        <div>
                ...
                <Navbar fluid rounded className='customBg w-4/5 navbarHeight'>
                    <Navbar.Brand >
                        <a className="flex items-center space-x-3 rtl:space-x-reverse">
                            <span className="self-center font50 text-wrap font-semibold whitespace-nowrap dark:text-white">Say Hello</span>
                        </a>
                    </Navbar.Brand>
                    <Navbar.Toggle />
                    <Dropdown label="Other Pages" className='w-11/12 font-bold' dismissOnClick={false}>
                        ...
                        <Dropdown.Item as={Link} to='/SayHello' className='block py-5 px-3 text-white hover:text-gray-900 bg-blue-700 rounded dark:bg-blue-600 font30 text-wrap'>Say Hello</Dropdown.Item>
            ...
            <div className="flex justify-center mt-12">
                <div className="grid grid-cols-1 gap-8 w-4/5 font30">
                    <div className="flex justify-center px-5 py-2.5 m-2">Give the computer your name and it will respond back with a "Hello".</div>
                    <div className="flex justify-center px-5 py-2.5 m-2"><input type="text" placeholder="Enter Name Here" className="rounded-md" onChange={e => SayHelloName(e.target.value)} /></div>
                    <div className="flex justify-center px-5 py-2.5 m-2"><button type="button" className="customBg hover:bg-gray-400 font-medium rounded-lg px-5 m-2" onClick={() => SH()}>Submit</button></div>
                    <div className="flex justify-center px-5 py-2.5 m-2">{result}</div>
                </div>
            </div>
        </div>
    )
